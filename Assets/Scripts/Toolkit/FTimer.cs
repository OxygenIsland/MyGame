...
namespace StarWorld.Common.Utility
{
    ...
    public class FTimer : MonoBehaviour
    {
        class EventData
        {
            ...
            public EventData(string k, float time, bool keep, Action func)
            {
                key = k;
                delay = time;
                isKeep = keep;
        ...
        protected static readonly bool isInit = false;
        ...
        /// <returns></returns>
        public static bool AddEvent(string eventId, float time, Action callback)
        {
            Init();
        ...
        private void OprateTimeUpEvent()
        {
            ...
            {
                if (!eDict.ContainsKey(eList[i].key))
                {
                    eList.RemoveAt(i);
                    continue;
                }
