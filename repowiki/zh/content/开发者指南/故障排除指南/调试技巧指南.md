...
LogScriptingDefineSymbols --> OhMyPackageLog : "控制输出"
```
...
## 依赖关系分析
- 日志系统
  - Log/Log门面依赖 OhMyPackageLog
  - OhMyPackageLog 依赖 ILogHelper，默认实现为 DefaultLogHelper
  - 编辑器侧通过宏定义控制日志输出
- 调试器模块
...
```mermaid
graph LR
LogFacade["Log/Log门面"] --> GFLog["OhMyPackageLog"]
